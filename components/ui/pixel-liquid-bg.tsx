...
void main(){
  ...
  float noiseVal = noise(uv * 6.0 + uTime * 0.15) * 0.04 - 0.02;

  float t = clamp(len + dither * 0.1 + noiseVal, 0.0, 1.0);

  vec3 fluidColor = texture2D(palette, vec2(t, 0.5)).rgb;
  vec3 col        = mix(bgColor.rgb, fluidColor, t * 0.72);

  float grain = hash(gl_FragCoord.xy + vec2(uTime * 137.0, uTime * 91.0));
  col += (grain - 0.5) * 0.035;

  float alpha = mix(bgColor.a, 0.85, t);
...
function getBgColor(dark: boolean) {
  return dark
    ? new THREE.Vector4(0.047, 0.039, 0.035, 0)
    : new THREE.Vector4(0.976, 0.969, 0.961, 0);
}
...
export interface PixelLiquidBgProps extends ComponentProps<"div"> {
  darkPalette?: string[];
  lightPalette?: string[];
  pixelSize?: number;
  resolution?: number;
...
export function PixelLiquidBg({
  ...
  useEffect(() => {
    ...
    const driver = autoDemo
      ? new AutoDriver(mouse, () => lastInteraction, 0.32, 1600)
      : null;
  ...
  return (
    <div
      ref={mountRef}
      className={cn("relative h-full w-full overflow-hidden bg-bg", className)}
      {...props}
    >
      {children && (
